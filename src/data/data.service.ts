...
import { HttpUtil } from './httpUtils.service';
...
export class DataService {
    ...
    async getRestaurantsList() {
        ...
        const response = await HttpUtil.get(url, headers)
        const nextPageOffSet = response?.data?.pageOffset
    ...
    async getRestaurantMenu(resId: any) {
    ...
    async getUpdatedRestaurantsList(body: any) {
        try {
            ...
            const response = await HttpUtil.post(url, data, headers)
            const nextPageOffSet = response?.data?.pageOffset
            return { pageOffSet: nextPageOffSet, restaurants: response?.data?.cards[0]?.card?.card?.gridElements?.infoWithStyle?.restaurants }
    ...
    async getDishes() {
        ...
        const response = await HttpUtil.get(url, headers)
        const title = response?.data?.cards[0]?.card?.card.header?.title;
        const dishes = response?.data?.cards[0]?.card?.card?.imageGridCards?.info
        return { title, dishes };
    }
